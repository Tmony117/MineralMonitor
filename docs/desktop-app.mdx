...
Mineral Monitor desktop uses a runtime configuration schema with per-feature toggles and secret-backed credentials.
...
- Mineral Monitor API key: cloud fallback is blocked; desktop operates local-only.
