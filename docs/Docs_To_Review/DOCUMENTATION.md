...
![Mineral Monitor Dashboard](images/new-mineral-monitor.png)

## Platform Variants

Mineral Monitor runs two specialized variants from a single codebase, each optimized for different monitoring needs:

| Variant | URL | Focus |
|---------|-----|-------|
| **🌍 Mineral Monitor** | [worldmonitor.app](https://worldmonitor.app) | Geopolitical intelligence, military tracking, conflict monitoring, infrastructure security |
...
The Service Status panel tracks the operational health of external services that Mineral Monitor users may depend on.
...
// App.ts
const header = `Mineral Monitor v${__APP_VERSION__}`;
```
...
- ✅ **Variant Switcher UI** - Compact orbital navigation between Mineral Monitor and Tech Monitor
